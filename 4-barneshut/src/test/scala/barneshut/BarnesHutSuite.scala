...
class BarnesHutSuite extends munit.FunSuite:
  ...
  test("Fork works for 4 emtpy quads") {
    val f = Fork(Empty(17.5f, 27.5f, 5.0f),
                 Empty(22.5f, 27.5f, 5.0f),
                 Empty(17.5f, 32.5f, 5.0f),
                 Empty(22.5f, 32.5f, 5.0f))
    assertEquals(f.massX, 20.0f, "Wololoo")
  }
  ...
  test("Fork with 3 empty quadrants and 1 leaf (nw)") {
    ...
    assertEquals(quad.centerX, 20f, s"${quad.centerX} should be 20f")
    assertEquals(quad.centerY, 30f, s"${quad.centerY} should be 30f")
    assertEquals(quad.mass, 123f, s"${quad.mass} should be 123f")
    assertEquals(quad.massX, 18f, s"${quad.massX} should be 18f")
    assertEquals(quad.massY, 26f, s"${quad.massY} should be 26f")
    assertEquals(quad.total, 1, s"${quad.total} should be 1")
  }
