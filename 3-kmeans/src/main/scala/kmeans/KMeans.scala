...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    var res: ParMap[Point, ParSeq[Point]] = points.groupBy(findClosest(_,means))
    for mean <- means do
      if (!(res contains mean))
        res = res.updated(mean, ParSeq())
    res
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
    // classified.map(findAverage)
    oldMeans.map(x => findAverage(x, classified(x)))

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    (oldMeans zip newMeans).forall(_.squareDistance(_) <= eta)

  @tailrec
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    val new_means = update(classify(points, means), means)
    if !converged(eta, means, new_means) then kMeans(points, new_means, eta) else new_means
